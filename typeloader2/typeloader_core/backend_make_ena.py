...
def transform(posHash):
    ...
    if len(utr):
        if utr[0][0] < -1:
            ...
            if len(utr) > 1:
                newPosHash["utr"] = [(1, utr[0][1] + offset), (utr[1][0] + offset, utr[1][1] + offset)]
            else:
                newPosHash["utr"] = [(1, utr[0][1] + offset), ()]
            ...
            newPosHash["introns"] = {number: (pos[0] + offset, pos[1] + offset) for (number, pos) in
                                     list(introns.items())}
            # add offset to all pseudoexons
            newPosHash["pseudoexons"] = {number: (pos[0] + offset, pos[1] + offset) for (number, pos) in
                                         list(pseudoexons.items())}
        else:
            # no 5' UTR found but 3' UTR exists
            newPosHash["exons"] = {number: (pos[0], pos[1]) for (number, pos) in list(exons.items()) if
                                   number != max_exon}
...
def make_header(backend_dict, general, enaPosHash, null_allele):
    ...
    general["{exon_coord_list}"] = ",".join(
        ["%s..%s" % (region[0], region[1]) for key, region in enaPosHash["cds"].items()])
...
def make_genemodel(backend_dict, general, enaPosHash, extraInformation, features):
    ...
    for featureIndex in range(len(features)):
        ...
        if feature[1].startswith("e"):
            ...
            genemodelop += eText.replace("{start}", str(exon[0])).replace("{stop}", str(exon[1])).replace("{exon_num}",
                                                                                                          str(exonNums[
                                                                                                                  number]))
            ...
            genemodelop += iText.replace("{start}", str(intron[0])).replace("{stop}", str(intron[1])).replace(
                "{intron_num}", str(intronNums[number]))
