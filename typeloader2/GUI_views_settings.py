...
class PasswordEditor(QDialog):
    ...
    def change_password(self):
        ...
        if new_pw != new_pw2:
            QMessageBox.warning(self, "Confirmation Error",
                                "New password does not match its confirmation. Please try again!")
...
class SettingTab(QTabWidget):
    ...
    def prepare_settings(self):
        ...
        self.mydic = {"login": {"section": "User",
                      ...
                      "fav_provenances": {"section": "Pref",
                                          "lbl_text": "Preferred Provenances",
                                          "hint": "These 'provenance' options will be listed above the rest. Must be separated by |, no whitespaces!"},
                      }

        try:
            self.cf["Pref"]["fav_provenances"]
        except KeyError:
            self.cf.set("Pref", "fav_provenances", "")
    ...
    def add_tab(self, text, sections):
        ...
        combobox_fields = {"address_form": ["Dr.", "Ms.", "Mrs.", "Mr.", "Ass. Prof.", "Prof.", "Med. Prof."],
                           "material_available": ["No Material Available", "DNA", "Whole Blood Sample",
                                                  ...
                                                  "Cell Line - Epithelial Cell Line",
                                                  "Cell Line - Fibroblast Cell Line",
                                                  "Cell Line - Histiocytic macrophage",
                                                  "Cell Line - Hodgkins Disease Cell Line",
                           ...
                           "primary_sequencing": ["NGS - Illumina Sequencing Technology",
                                                  "NGS - Pacific Biosciences SMRT Technology",
                                                  "NGS - Oxford Nanopore Technology",
                                                  "NGS - Ion Torrent Sequencing Technology",
                           ...
                           "secondary_sequencing": ["NGS - Illumina Sequencing Technology",
                                                    "NGS - Pacific Biosciences SMRT Technology",
                                                    "NGS - Oxford Nanopore Technology",
                                                    "NGS - Ion Torrent Sequencing Technology",
                                                    "NGS - 454 Sequencing Technology",
                                                    "Direct sequencing of PCR product from DNA (SBT)",
                                                    "Direct sequencing of PCR product from RNA (SBT)",
                                                    "Cloning of PCR product & sequencing",
...
class UserSettingsDialog(QDialog):
    ...
    def value_ok(self, field, value):
        ...
        if field in ["cell_line_token", "ipd_shortname"]:
            ...
            if len(value) > 10:
                QMessageBox.warning(self, "{} rejected".format(mydic[field]),
                                    "Your {} is too long.\nPlease restrict yourself to max. 10 characters!".format(
                                        mydic[field]))
                return False
    ...
    def closeEvent(self, event):
        ...
        if self.unconfirmed_changes:
            QMessageBox.warning(self, "Unsaved changes",
                                "You have unsaved changes. Please confirm or reset them before leaving!")
            event.ignore()
        else:
            event.accept()
