...
def make_globaldata(species="Homo sapiens", gene="", allele="", partial="", product="",seqLen="",cellline="",codonstart="1"):
   
    globaldata = {"{species}":species,"{gene}":gene,"{allele}":allele,"{partial}":partial,"{product}":product,"{sequence length}":seqLen,"{cell line}":cellline,"{reading frame}":codonstart}
    return globaldata
...
def transform(posHash):
    ...
    if len(utr):
        if utr[0][0] < -1:
            ...
            if len(newPosHash["utr"][1]): newPosHash["exons"].append((exons[-1][0]+offset,newPosHash["utr"][1][1]))
...
def make_footer(fText, sequence,seqwidth=80):
