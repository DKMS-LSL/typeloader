...
def changeToImgtCoords(features, coordinates, differences, utr5Length = 0):
    ...
    cdsMap = constructCDS(features, imgtCoordinates)
    
    imgtDifferences["deletionPositions"] = [transformPos(deletionPos, cdsMap) for deletionPos in imgtDifferences["deletionPositions"]]
    imgtDifferences["insertionPositions"] = [transformPos(insertionPos, cdsMap) for insertionPos in imgtDifferences["insertionPositions"]]
    imgtDifferences["mismatchPositions"] = [transformPos(mismatchPos, cdsMap) for mismatchPos in imgtDifferences["mismatchPositions"]]
